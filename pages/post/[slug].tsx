    ...
    <main>
        ...
        {submitted ? (
            <div className='flex flex-col p-10 my-10 bg-green-500 text-white max-w-2xl mx-auto'>
                ...
                <p>Please wait for moderator to approve your comment</p>
            </div>
            
        ): (
            <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col p-5 max-w-2xl mx-auto mb-10'>
            <h3 className='text-sm text-green-500'>Do you have something to say about the article?</h3>
            ...
            <div className='flex flex-col p-5'>
                ...
                {errors.email && (<span className='text-red-500'>Field is required</span>)}
                {errors.comment && (<span className='text-red-500'>Field is required</span>)}
            </div>
       ...
       <div className='flex flex-col p-10 my-10 max-w-2xl mx-auto shadow-green-500 shadow space y-2'>
           ...
           {post.comments.map((comment) => (
               <div key={comment._id}>
                   <p className=''><span className='text-green-500'>{comment.name}:</span> {comment.comment}</p>
               </div>
           ))}
       </div>
    </main>
    ...
    const query = `*[_type == "post" && slug.current == $slug] [0]{
      ...
      const post = await sanityClient.fetch(query,
        ...
        return{
            ...
            revalidate: 90, //after 90 seconds it will update the old cached version
        };
};